...
$('#analysisForm').onsubmit=async e=>{
  ...
  try{
    lastAnalysis=await api('/api/analyze',{method:'POST',body:JSON.stringify({symbol:$('#symbol').value,exchange:$('#exchange').value,execution_timeframe:$('#executionTf').value,balance:$('#balance').value,risk_pct:$('#risk').value})});
...
function renderAnalysis(a){
  ...
  if(cand){
    ...
    h+=`</div><div style="margin-top:10px"><button class="ghost" id="trackCandidate">أضف للمراقبة بضغطة</button></div>`;
  }
...
async function trackCandidate(){
  ...
  try{await api('/api/trades',{method:'POST',body:JSON.stringify(lastAnalysis.candidate.tracking_payload)});$('#trackCandidate').textContent='تمت الإضافة ✓';await loadTrades();}
  catch(e){showError(e.message)}
}

const statusAr={watchlist:'مراقبة',pending_entry:'انتظار دخول',active:'مفتوحة',runner:'Runner بعد TP1',stopped:'ستوب',tp2_hit:'TP2 تحقق',closed:'مغلقة',cancelled:'ملغاة'};
async function loadTrades(){
  try{const r=await api('/api/trades');const trades=r.trades||[];$('#tradeCount').textContent=trades.filter(t=>!['closed','cancelled','stopped','tp2_hit'].includes(t.status)).length;renderTrades(trades)}catch(e){$('#tradesList').innerHTML=`<div class="error-box">${esc(e.message)}</div>`}
}
function renderTrades(trades){
  ...
  <div class="actions">${t.status==='watchlist'?`<button class="mini" onclick="tradeAction('${esc(t.id)}','activate')">تفعيل انتظار الدخول</button>`:''}<button class="mini" onclick="tradeAction('${esc(t.id)}','refresh')">تحديث</button>${!['closed','cancelled','stopped','tp2_hit'].includes(t.status)?`<button class="mini danger" onclick="tradeAction('${esc(t.id)}','cancel')">إلغاء</button>`:''}</div>
